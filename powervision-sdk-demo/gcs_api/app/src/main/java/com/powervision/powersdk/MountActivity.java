    ...
    private void initClickListener() {
        ...
        setBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mConnectStatus != 2) {
                    Toast.makeText(MountActivity.this, R.string.device_dis_connect, Toast.LENGTH_SHORT).show();
                    return;
                }
...
//        supportRequestWindowFeature()
    ...
    private void setI2cParam() {
        if (i2cMacAddress.length() == 0) {
            ToastUtil.showToast(getString(R.string.ip_content_not_non));
            return;
        }
...
//        mPowerSDK.setSpiParam(currentSpiBps, currentSpiMode, currentSpiFirstBit, currentSpiDataSize, currentSpiCrcEnable);
        ...
        if (peridRadio.length() == 0 || lowLastTime.length() == 0 || highLastTime.length() == 0) {
            ToastUtil.showToast(getString(R.string.content_not_non));
            return;
        }
    ...
    private void startPowerSDK() {
        mPowerSDK.startConnectSDK(ConnectIpAndPortFactory.getEyeConnectIpAndPortFactory());// ip = "192.168.1.12"; port = 20002;
...
//        mPowerSDK.startDisconnectSDK();
    ...
    ConnectListener.SimpleConnectListener simpleConnectListener = new ConnectListener.SimpleConnectListener() {
        ...
        public void onChainConnected() {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    statusText.setText(R.string.link_connected);
                    statusText.setTextColor(ContextCompat.getColor(MountActivity.this, R.color.connecting));
                }
            });
        }
        ...
        public void onDroneConnected() {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    statusText.setText(R.string.device_connected);
                    statusText.setTextColor(ContextCompat.getColor(MountActivity.this, R.color.black));
                }
            });
        }
        ...
        public void canFilter(final MountApiCanFilterParam[] params) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, sb.toString());
                }
            });
        }
    };
    ...
    MountDeviceListener.UartListener uartListener = new MountDeviceListener.UartListener() {
        ...
        public void onUartParamSet(final int state, final int error) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (state == 0) {
                        ToastUtil.showToast("success： " + error);
                    } else {
                        ToastUtil.showToast("failed： " + error);
                    }
                }
            });
        }
        ...
        public void onUartParamInquire(final int bps, final int dataBits, final int stopBits, final int parity, final int flowCtrl, final int state) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this,
                            "uartBps：" + uartBps[bps] + "\n uartStopBits：" + uartStopBits[stopBits] + "\n uartControl：" + uartControl[flowCtrl] +
                                    "\n uartBits：" + uartBits[dataBits] + "\n canCheck：" + canCheck[parity] + "\n state:" + state);
                }
            });
        }
        ...
        public void onUartDataReceive(final MountData data) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, "UART:" + new Gson().toJson(data));
                }
            });
        }
    };
    ...
    MountDeviceListener.CanListener canListener = new MountDeviceListener.CanListener() {
        ...
        public void onCanParamInquire(final int bps, final int mode) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, "canBps：" + canBps[bps] + "\n canModes：" + canModes[mode]);
                }
            });
        }
        ...
        public void onCanDataReceive(final MountApiCanData data) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    StandardDialogUtils.defaultDialog(MountActivity.this, "CAN:" + new Gson().toJson(data));
                }
            });
        }
    };
        ...
        public void onI2cParamInquire(final int bps, final int mode, final int deviceAddr) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, "i2cBps：" + i2cBps[bps] + "\n i2cModes：" + i2cModes[mode] + "\n deviceAddr：" + deviceAddr);
                }
            });
        }
        ...
        public void onI2cDataRead(final MountData data) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, "read I2C:" + new Gson().toJson(data));
                }
            });
        }
    };
        ...
        public void onSpiParamInquire(final int bps, final int mode, final int firstBit, final int dataSize, final int crcEnable) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this,
                            "spiModes：" + spiModes[mode] + "\n spiBps：" + spiBps[bps] + "\n spi_firstBit：" + spi_firstBit[firstBit] +
                                    "\n spi_dataSize：" + spi_dataSize[dataSize] + "\n spi_crcEnable：" + spi_crcEnable[crcEnable]);
                }
            });
        }
        ...
        public void onSpiDataRead(final MountData data) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this, "read SPI:" + new Gson().toJson(data));
                }
            });
        }
    };
        ...
        public void onGpioParamInquire(final int deviceNumber, final int periodRatio, final int periodLow, final int periodHigh) {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    StandardDialogUtils.defaultDialog(MountActivity.this,
                            "GPIOdeviceNum：" + deviceNum[deviceNumber] + "\n periodRatio：" + periodRatio + "\n periodLow：" + periodLow +
    ...
    private void showPortDialog(final int uart, final int can, final int i2c, final int spi, final int gpio1, final int gpio2, final int gpio3) {
        ...
        builder.setTitle(R.string.current_port_status);
